...
static void DETECTOR_TSK(void *p_arg)
{
    ...
    // implement interference detection using performance counter
    // access to floatSwitch that is on when water level falls below low water level threshold
    // need to raise appropriate signal accessed by controller
    // signal 0 => normal mode
    // signal 1 => graceful recovery mode
    // signal 2 => hard recovery mode
    ...
    static void CONTROLLER_TSK(void *p_args)
    {
        OS_ERR os_err;
        (void)p_args;
        // implement controller modes of operation
        // access to signal variable
        // signal 0 => normal mode
        // signal 1 => graceful recovery mode
        // signal 2 => hard recovery mode
